...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll($requestdata , $limit = null)
    {
        ...
        //Need the Optimization of Queries Needed Have to check frontend side.
        //if He has Authenticated Token then you can get All the User Information
        $user = Auth::user();

        if ($user->isSuperAdmin()) {
            $allJobs = Job::query();
            if (isset($requestdata['id']) && !empty($requestdata['id'])) {
                //Use Ternary Operator To handle it.
        ...
        //No Need to Get Full Data of User Just If you Need Id Get Id's.
            if (!empty($requestdata['customer_email'])) {
                $userIds = DB::table('users')->whereIn('email', $requestdata['customer_email'])->pluck('id') ->all();
                if (!empty($userIds)) {
                    $allJobs->whereIn('user_id', $userIds);
                }
            }
        //Problem In Writing Query Commonly Get All Data even if it is not needed.
            if (!empty($requestdata['translator_email'])) {
                $translatorEmails = $requestdata['translator_email'];
                $userIds = DB::table('users')->whereIn('email', $translatorEmails)->pluck('id')->all();
                if (!empty($userIds)) {
                    $allJobIDs = DB::table('translator_job_rel')->whereNull('cancel_at')->whereIn('user_id', $userIds)->pluck('job_id')->all();
                    if (!empty($allJobIDs)) {
